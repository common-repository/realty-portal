...
if ( !function_exists( 'noo_shortcode_pricing_table' ) ) :
    ...
    function noo_shortcode_pricing_table( $atts ) {
        ...
        extract( shortcode_atts( array(
            'featured_item' => '3',
            'button_txt'    => esc_html__( 'Buy now', 'realty-portal' )
        ), $atts ) );
        ...
        if ( $membership_query->have_posts() ) :
             ...
             */
                ...
                $interval_text_arr = array(
                    'day'   => array( esc_html__( 'Day', 'realty-portal' ), esc_html__( 'Days', 'realty-portal' ) ),
                    'week'  => array( esc_html__( 'Week', 'realty-portal' ), esc_html__( 'Weeks', 'realty-portal' ) ),
                    'month' => array( esc_html__( 'Month', 'realty-portal' ), esc_html__( 'Months', 'realty-portal' ) ),
                    'year'  => array( esc_html__( 'Year', 'realty-portal' ), esc_html__( 'Years', 'realty-portal' ) ),
                );
            ...
            while ( $membership_query->have_posts() ) : $membership_query->the_post();
                ...
                if ( !empty( $properties_num_unlimited ) ) {
                    ...
                    $properties_num     = esc_html__( 'Unlimited', 'realty-portal' );
                    $class_unlimited = 'unlimited';

                }
                if ($properties_num == 1) {
                    $limit_property        = !empty( $properties_num ) ? wp_kses( sprintf( __( '<span%s>%s</span> Property', 'realty-portal' ), ' class="' . esc_attr( $class_unlimited ) . '"', esc_html( $properties_num ) ), noo_allowed_html() ) : '';
                }else{
                $limit_property        = !empty( $properties_num ) ? wp_kses( sprintf( __( '<span%s>%s</span> Properties', 'realty-portal' ), ' class="' . esc_attr( $class_unlimited ) . '"', esc_html( $properties_num ) ), noo_allowed_html() ) : '';
                }

                $featured_num          = get_post_meta( $package_id, 'noo_membership_featured_num', true );
                if ($featured_num  <= 0 ) {
                    $limit_featured        = ( !empty( $featured_num ) || $featured_num == 0 ) ? wp_kses( sprintf( __( '<span>%s</span> Featured Property', 'realty-portal' ), esc_html( $featured_num ) ), noo_allowed_html() ) : '';
                }else{
                $limit_featured        = ( !empty( $featured_num ) || $featured_num == 0 ) ? wp_kses( sprintf( __( '<span>%s</span> Featured Properties', 'realty-portal' ), esc_html( $featured_num ) ), noo_allowed_html() ) : '';
                }
                ...
                ?>
                <div class="<?php echo esc_attr( $class ); ?> noo-pricing-table-item-wrap<?php echo ( absint( $featured_item ) == $count ? ' is_featured' : '' ) ?>">
                        ...
                        <ul class="pricing-body">
                            <?php
                                ...
                                if ( !empty( $expired_date ) ) {
                                    echo '<li class="expired-date">' . sprintf( esc_html__( '%s %ss Listing Duration', 'realty-portal' ), $expired_date, $expired_unit_date ) . '</li>';
                                }
            ...
            echo esc_html__( 'Sorry, no package matched your criteria.', 'realty-portal' );
