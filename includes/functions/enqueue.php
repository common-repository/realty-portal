...
if ( ! function_exists( 'realty_portal_script' ) ) :
	...
	function realty_portal_script() {
		...
		wp_register_script( 'jquery-blockui', REALTY_PORTAL_ASSETS . '/vendor/jquery-blockui/jquery.blockUI.min.js', array( 'jquery' ), null, false );
		...
		wp_register_script( $name_script_map, 'http' . ( is_ssl() ? 's' : '' ) . '://maps.googleapis.com/maps/api/js?v=3.exp&libraries=places' . ( ! empty( $google_api ) ? '&key=' . $google_api : '' ), array( 'jquery' ), '1.0', false );
...
if ( ! function_exists( 'realty_portal_script_admin' ) ) :
	...
	function realty_portal_script_admin() {
        ...
        wp_register_script( 'rp-quick-setup', REALTY_PORTAL_ASSETS . '/js/rp-admin-quick-setup.js', array(), null, true );
		wp_register_script( 'rp-addon', REALTY_PORTAL_ASSETS . '/js/addon.js', array( 'jquery' ), null, true );
        wp_localize_script('rp-quick-setup','RP_Property_Admin', array('ajax_url' =>admin_url('admin-ajax.php')));
