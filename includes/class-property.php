...
if ( ! class_exists( 'RP_Property' ) ) :
	...
	class RP_Property {
		...
		public static function query( $args = array() ) {
			...
			$meta_query = array();
			if ( isset( $_GET[ 'area_min' ] ) && ! empty( $_GET[ 'area_min' ] ) ) {
				$min_area[ 'key' ]     = 'noo_property_area';
				$min_area[ 'value' ]   = intval( $_GET[ 'area_min' ] );
				$min_area[ 'type' ]    = 'NUMERIC';
				$min_area[ 'compare' ] = '>=';
				$meta_query[]          = $min_area;
			}
			if ( isset( $_GET[ 'area_max' ] ) && ! empty( $_GET[ 'area_max' ] ) ) {
				$max_area[ 'key' ]     = 'noo_property_area';
				$max_area[ 'value' ]   = intval( $_GET[ 'area_max' ] );
		...
		public static function tax_query( $REQUEST, $filter = 'term_id' ) {

			$tax_query = array();
			$tax_list  = array(
				'types'  => apply_filters( 'rp_property_listing_type', 'listing_type' ),
				'offers' => apply_filters( 'rp_property_listing_offers', 'listing_offers' ), // offers - not status
			);
