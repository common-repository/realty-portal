...
class RP_Properties_Factory {
	...
	public function agent_info( $show = 'id' ) {
		...
		switch ( $show ) {
			...
			case 'address':
				$data_agent = get_post_meta( $this->agent_info(), 'rp_agent_address', true );
				break;
	...
	public function agent_avatar( $size = 'rp-agent-avatar-medium', $echo = true ) {
		$id_avatar = get_post_thumbnail_id( $this->agent_info() );
		$avatar    = rp_thumb_src_id( $id_avatar, $size, $size );
		if ( $echo ) {
	...
	public function get_listing_offers_html( $prefix = '' ) {
		...
		if ( ! empty( $listing_offers ) ) {
			...
			foreach ( $listing_offers as $status ) {
				...
				$list_status[] = sprintf( '<a href="%1$s" title="%2$s" style="background: %3$s;">%2$s</a>', esc_url( get_term_link( $status->slug, apply_filters( 'rp_property_listing_offers', 'listing_offers' ) ) ), esc_html( $status->name ), $color );
			}
	...
	public function price() {
		...
		return apply_filters( 'rp_price', absint( $price ) );
	}
