		...
		switch ( $column_name ) {
			...
			case 'rp_actions':
				...
				if ( $post->post_status !== 'trash' ) {
					...
					if ( Realty_Portal::is_plugin_active( 'duplicate-page/duplicatepage.php' ) ) {
						if (current_user_can('edit_posts')) {
							$admin_actions[ 'duplicate' ] = array(
								'icon'     => 'rp-icon-clone',
								'name'     => __( 'Duplicate', 'realty-portal' ),
								'url'      => 'admin.php?action=dt_duplicate_post_as_draft&amp;post=' . $post->ID,
								'cap'      => 'edit_posts',
								'priority' => 35,
							);
						}
