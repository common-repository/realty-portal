	...
	function rp_loadmore_property_request() {
		...
		if ( $wp_query->have_posts() ) {
			if ( ! empty( $style_show_ajax )  ) {
				...
				<div class="noo-count">
					<?php echo sprintf( esc_html__( '%s listings found', 'realty-portal' ), $wp_query->found_posts ) ?>
				</div>
				<form class="sort-property">
					<?php echo esc_html__( 'Sort By:', 'realty-portal' ); ?>
					<select name="orderby">
						<option value="date"<?php selected( $orderby, 'date', true ); ?>>
							<?php echo esc_html__( 'Date', 'realty-portal' ); ?>
						</option>
						<option value="price"<?php selected( $orderby, 'price', true ); ?>>
							<?php echo esc_html__( 'Price', 'realty-portal' ); ?>
						</option>
						<option value="bath"<?php selected( $orderby, 'bath', true ); ?>>
							<?php echo esc_html__( 'Bath', 'realty-portal' ); ?>
						</option>
						<option value="bed"<?php selected( $orderby, 'bed', true ); ?>>
							<?php echo esc_html__( 'Bed', 'realty-portal' ); ?>
						</option>
						<option value="area"<?php selected( $orderby, 'area', true ); ?>>
							<?php echo esc_html__( 'Area', 'realty-portal' ); ?>
						</option>
						<option value="featured"<?php selected( $orderby, 'featured', true ); ?>>
							<?php echo esc_html__( 'Featured', 'realty-portal' ); ?>
						</option>
						<option value="name"<?php selected( $orderby, 'name', true ); ?>>
							<?php echo esc_html__( 'Name', 'realty-portal' ); ?>
						</option>
					</select>
			...
			endwhile;
			test_ajax_pagination($wp_query,$current_page);
			wp_reset_postdata();
			// rp_property_loop_end();
			?>
			<div class="noo-results-property"></div>
			<?php
